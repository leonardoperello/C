...
int main() {
	...
	*/
	...
	Sensor* temperaturaAire = newSensor(15,"Temperatura del aire", 20.0, 30.0);
	Sensor* humedadAire = newSensor(15,"Humedad del aire", -1, 75);

	const int sizeHumedadSueloArray = 5;
	...
	operar(humedadAire, iteraciones, doublePorReg);
		...
		for (i = 0; i < sizeHumedadSueloArray; i++) {
			sensoresHumedadSuelo[i] = newSensor(15,"Humedad del suelo", -1, 30);
		}
