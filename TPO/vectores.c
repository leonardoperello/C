...
/*			PARA USAR EN LINUX. SE USA POSIX	*/
	...
	if (posix_memalign((void**)&sensor->input, 16, size * sizeof(double)) != 0)
		return 1;

	if (posix_memalign((void**)&sensor->result, 16, size * sizeof(double)) != 0)
		return 1;
	...
	*/
	...
	operar(humedadAire, iteraciones, doublePorReg);
		...
		for (i = 0; i < sizeHumedadSueloArray; i++) {
			sensoresHumedadSuelo[i] = newSensor(10,"Humedad del suelo", -1, 30);
		}
